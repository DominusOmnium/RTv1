...
void			initialization_physical_device()
{
...
void			vku_get_physical_device(t_vulkan *v)
{
	...
	while (i < num_d)
	{
		...
		while (++j < num)
		{
			vkGetPhysicalDeviceSurfaceSupportKHR(d[i], j, v->surface,
															&(sup_pres[j]));
			if (qf_prop[j].queueFlags & VK_QUEUE_GRAPHICS_BIT &&
				d_prop[i].deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU)
				if (sup_pres[j])
				{
					...
					vkGetPhysicalDeviceSurfaceFormatsKHR(d[i], v->surface,
															&(num), NULL);
					...
					vkGetPhysicalDeviceSurfaceFormatsKHR(d[i], v->surface, &num,
												v->phys_device.surface_formats);
					vkGetPhysicalDeviceSurfaceCapabilitiesKHR(d[i], v->surface,
...
VkDeviceCreateInfo			initialization_dev_info(t_vulkan *v,
...
void			vku_create_logical_device(t_vulkan *v)
{
	...
	dev_info = initialization_dev_info(v, dev_ext, queue_priorities);
