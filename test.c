...
VkInstanceCreateInfo createInfo = {
    ...
    .ppEnabledExtensionNames = (const char* const[]) { VK_KHR_SURFACE_EXTENSION_NAME, VK_KHR_XCB_SURFACE_EXTENSION_NAME },
};
...
const VkInstanceCreateInfo createInfoLunarGValidation = {
    ...
    .ppEnabledExtensionNames = (const char* const[]) { VK_KHR_SURFACE_EXTENSION_NAME, VK_KHR_XCB_SURFACE_EXTENSION_NAME, VK_EXT_DEBUG_REPORT_EXTENSION_NAME },
};
...
int init_vulkan()
{
	...
	createInfo.enabledExtensionCount = count_ext;
    createInfo.ppEnabledExtensionNames = inst_ext;
#ifdef VULKAN_ENABLE_LUNARG_VALIDATION1
	...
	printf("Instance creation result: %d\n", result);
...
int init_window()
{
    SDL_Window* window = SDL_CreateWindow("Vulkan Sample", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, SDL_WINDOW_VULKAN
    );
	if (window == NULL)
	{
		printf("Window error\n");
	...
	{
		printf("SDL_Vulkan_CreateSurface error\n");
		return (0);
	}
...
int init_device()
{
    ...
    for (uint32_t i = 0; i < physicalDeviceCount; ++i)
    {
		...
		vkGetPhysicalDeviceProperties(deviceHandles[i], &prop);
        ...
        for (uint32_t j = 0; j < queueFamilyCount; ++j) {
            ...
            if (supportsPresent && (queueFamilyProperties[j].queueFlags & VK_QUEUE_GRAPHICS_BIT) && prop.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU)
            {
                queueFamilyIndex = j;
...
static size_t	read_shader_file(char *fname, char **shader)
{
	...
	{
		*shader = realloc(*shader, nor + r);
		memcpy(*shader + nor, line, r);
		nor += r;
	}
	close(fd);
	return (nor);
...
VkShaderModule	vku_createShaderModule(char *code, uint32_t codeL)
{
	...
	VkResult res;
	if ((res = vkCreateShaderModule(device, &smCreateInfo, NULL, &shaderModule)) != VK_SUCCESS)
	{
		printf("%d\n", res);
		return (0);
	}
...
int createPipeline()
{
    ...
    VkShaderModule fragmentShader = vku_createShaderModule(code, code_len);

	code_len = load_shader_file("shaders/testver.spv", &code);
    VkShaderModule vertexShader = vku_createShaderModule(code, code_len);
    ...
    VkResult res = vkCreatePipelineLayout(device, &pipelineLayoutCreateInfo, 0, &pipelineLayout);
    ...
    res = vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineCreateInfo, 0, &pipeline);
...
int createUploadBuffer()
{
    ...
    VkResult res = vkCreateBuffer(device, &bufferCreateInfo, NULL, &uploadBuffer);
    ...
    res = vkAllocateMemory(device, &allocInfo, NULL, &uploadBufferMemory);
